...
function SessionSummarySection({ title, items }: { title: string; items: string[] }) {
  ...
  return (
    <box flexDirection="column" style={{ paddingLeft: 1, paddingTop: 1 }}>
      ...
      {items.map((item, i) => (
        <text key={`${title}-${i}`} fg={THEME.text}>  {item}</text>
      ))}
    </box>
  );
}

function SessionSummaryView({ session, summary }: { session: SessionInfo; summary: SessionActivitySummary }) {
  return (
      ...
      {summary.finalResult ? (
        <box style={{ paddingLeft: 1, paddingTop: 1 }}>
          <text>
            ...
            <span fg={THEME.text}>{'  '}{summary.finalResult}</span>
          </text>
        </box>
      ) : null}
      {summary.finalText ? (
        <box style={{ paddingLeft: 1, paddingTop: 1 }}>
          ...
          <text fg={THEME.text}>  {summary.finalText}</text>
        </box>
      ) : null}
      <SessionSummarySection title="Verification" items={summary.verification} />
      <SessionSummarySection title="Bash Commands" items={summary.bashCommands} />
      <SessionSummarySection title="File Operations" items={summary.fileOps} />
...
export function SessionDetail({ session, onBack, onQuit, onTabSwitch, interactive = true, showFooter = true }: {
  session: SessionInfo;
  onBack: () => void;
  onQuit: () => void;
  onTabSwitch: () => void;
  ...
  return (
    <box flexDirection="column">
      ...
      {interactive ? (
        ...
        >
          <SessionSummaryView session={session} summary={activitySummary} />
        </scrollbox>
      )}
      {showFooter ? (
